...
class Information:
    ...
    async def help(self, ctx):
        ...
        embed = embed.set_thumbnail(url="https://raw.githubusercontent.com/Annihilator708/DiscordBot-BotZilla/master/icon.png")
        embed = embed.add_field(name="{}8ball".format(self.config['prefix']), value="8ball! Ask BotZilla Any question.", inline=True)
        embed = embed.add_field(name="{}Joke".format(self.config['prefix']), value="Ever heard a Chuck Norris joke?", inline=True)

        embed = embed.add_field(name="{}count".format(self.config['prefix']), value="Count servers, members, etc.", inline=True)
        embed = embed.add_field(name="{}fact".format(self.config['prefix']), value="Search for a fact!", inline=True)

        embed = embed.add_field(name="{}id".format(self.config['prefix']), value="Shows your ID or the id of the user.", inline=True)
        embed = embed.add_field(name="{}gif".format(self.config['prefix']), value="Retrieves a random gif from a giphy search", inline=False)
        embed = embed.add_field(name="{}rule34".format(self.config['prefix']), value="Shows graphical content NSFW.", inline=False)
        embed = embed.add_field(name="{}emoji".format(self.config['prefix']), value="Shows ASCII information about the emoji", inline=False)

        embed = embed.add_field(name="{}ping".format(self.config['prefix']), value="Check server response", inline=True)
        embed = embed.add_field(name="{}poll".format(self.config['prefix']), value="Make a poll for your server", inline=True)
        embed = embed.add_field(name="{}join".format(self.config['prefix']), value="Add BotZilla to your server!", inline=True)
        embed = embed.add_field(name="{}blacklist".format(self.config['prefix']), value="Global blacklist vote", inline=True)
        embed = embed.add_field(name="{}r6s".format(self.config['prefix']), value="Shows your Rainbow Six Siege stats", inline=True)
        embed = embed.add_field(name="{}bitcoin".format(self.config['prefix']), value="Shows current bitcoin value", inline=True)
        embed = embed.add_field(name="{}summon".format(self.config['prefix']), value="Summons the bot to join your voice channel", inline=False)
        embed = embed.add_field(name="{}stop".format(self.config['prefix']), value="Stops playing audio and leaves the voice channel", inline=False)
        embed = embed.add_field(name="{}play".format(self.config['prefix']), value="Plays a song", inline=False)
        embed = embed.add_field(name="{}pause".format(self.config['prefix']), value="Pauses the currently played song", inline=False)
        embed = embed.add_field(name="{}resume".format(self.config['prefix']), value="Resumes the currently played song", inline=False)
        embed = embed.add_field(name="{}np".format(self.config['prefix']), value="Shows info about the currently played song", inline=False)
        embed = embed.add_field(name="{}skip".format(self.config['prefix']), value="Vote to skip a song. The song requester can always skip his own song", inline=False)
        embed = embed.add_field(name="{}volume".format(self.config['prefix']), value="Sets the volume of the currently playing song", inline=False)
        embed = embed.set_footer(text="BotZilla is built / maintained / self hosted by PuffDip\nUserdata may be stored for better experience.")
        await self.bot.say(embed=embed)
