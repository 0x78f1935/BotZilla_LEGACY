    ...
    async def steal(self, ctx, item = None):
        ...
        if user_choice in str(item):
            ...
            if int(jail_number) >= int(item[0][9]):
                ...
                query = "INSERT INTO botzilla.c_user(XP, score, LVL, money, jail, jail_date) VALUES({}, {}, {}, {}, {}, {}) WHERE ID = '{}'".format(experience, score, level, money, jail, jail_date, ctx.message.author.id)
                self.database.cur.execute(query)
                self.database.conn.commit()
                self.database.cur.execute("ROLLBACK;")
